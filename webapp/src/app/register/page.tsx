          ...
          <form action={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4 mb-2">
              <div 
                ...
                className={`cursor-pointer flex flex-col items-center p-3 rounded-xl border transition-all ${type === 'singolo' ? 'border-yellow-500 bg-yellow-500/10' : 'border-slate-300 bg-white/50 hover:bg-slate-100'}`}
              >
                <User className={`w-6 h-6 mb-1 ${type === 'singolo' ? 'text-yellow-600' : 'text-slate-500'}`} />
                <span className="text-sm font-medium text-slate-700">Freelance</span>
              </div>
              <div 
                onClick={() => setType('salone')}
                className={`cursor-pointer flex flex-col items-center p-3 rounded-xl border transition-all ${type === 'salone' ? 'border-yellow-500 bg-yellow-500/10' : 'border-slate-300 bg-white/50 hover:bg-slate-100'}`}
              >
                <Building className={`w-6 h-6 mb-1 ${type === 'salone' ? 'text-yellow-600' : 'text-slate-500'}`} />
            ...
            <div className="grid grid-cols-2 gap-4">
              <input
                ...
                required
                placeholder="Nome"
                className="w-full bg-white/50 border border-slate-300 rounded-xl py-3 px-4 text-slate-900 placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:border-transparent transition-all"
              />
              <input
                name="cognome"
                required
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full mt-4 bg-gradient-to-r from-yellow-500 to-yellow-600 hover:from-yellow-400 hover:to-yellow-500 text-white rounded-xl py-3 font-semibold shadow-lg shadow-yellow-500/25 flex items-center justify-center gap-2 transition-all active:scale-[0.98] disabled:opacity-70"
            >
              {loading ? (
